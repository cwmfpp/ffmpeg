...
{//cwm 
    int64_t curpos = avio_tell(pb);//cwm curpos=28 pos=0
...
static int mov_write_stco_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32(pb, 0); /* size */
    if (mode64)//cwm mode64=0
        ffio_wfourcc(pb, "co64");
    else
        ffio_wfourcc(pb, "stco");//cwm true
    ...
    for (i = 0; i < track->entry; i++) {
        ...
        if (mode64 == 1)
            ...
            avio_wb32(pb, track->cluster[i].pos + track->data_offset);//track->cluster[i].pos第一个的pos值为44，track->data_offset=0
    }
...
{//说明samples的个数，以及每个samples数据的长度
    ...
    for (i = 0; i < track->entry; i++) {//track->entry = 232
        tst = track->cluster[i].size / track->cluster[i].entries;
        if (oldtst != -1 && tst != oldtst)
            equalChunks = 0;
        oldtst = tst;
        entries += track->cluster[i].entries;
    }//cwm entries = 232 equalChunks=0
    if (equalChunks && track->entry) {//cwm equalChunks=0
...
static int mov_write_stss_tag(AVIOContext *pb, MOVTrack *track, uint32_t flag)
{
    ...
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); // size
    ffio_wfourcc(pb, flag == MOV_SYNC_SAMPLE ? "stss" : "stps");//cwm flag=MOV_SYNC_SAMPLE,"stss"
    avio_wb32(pb, 0); // version & flags
    entryPos = avio_tell(pb);
...
{
    int64_t pos = avio_tell(pb);//2393393=0x248531

    avio_wb32(pb, 0);
    ffio_wfourcc(pb, "hvcC");
    ff_isom_write_hvcc(pb, track->vos_data, track->vos_len, 0);//track->vos_len = 82
    return update_size(pb, pos);//cwm size=116
}
...
static int mov_write_video_tag(AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    avio_wb32(pb, 0); /* size *///cwm 0xca
    avio_wl32(pb, track->tag); // store it byteswapped //cwm hev1
    avio_wb32(pb, 0); /* Reserved */
    avio_wb16(pb, 0); /* Reserved */
    avio_wb16(pb, 1); /* Data-reference index */
    ...
    if (track->mode == MODE_MOV) {//cwm false
    ...
    avio_wb16(pb, track->enc->width); /* Video width */ //cwm 1280=0x500
    avio_wb16(pb, track->height); /* Video height */ //cwm 720=0x2d0
    ...
    avio_w8(pb, strlen(compressor_name));//cwm 0,到此处4字节对齐了
    avio_write(pb, compressor_name, 31);//cwm 0

    if (track->mode == MODE_MOV && track->enc->bits_per_coded_sample)
        avio_wb16(pb, track->enc->bits_per_coded_sample);
    else
        avio_wb16(pb, 0x18); /* Reserved *///cwm true
    ...
    if (track->tag == MKTAG('m','p','4','v'))
        ...
        mov_write_hvcc_tag(pb, track);//cwm  true hvcC
    ...
    if (track->enc->codec_id != AV_CODEC_ID_H264 &&
        ...
        track->enc->codec_id != AV_CODEC_ID_DNXHD)//cwm false
        if (track->enc->field_order != AV_FIELD_UNKNOWN)
            mov_write_fiel_tag(pb, track);

    if (mov->flags & FF_MOV_FLAG_WRITE_GAMA) {//cwm false
        if (track->mode == MODE_MOV)
            mov_write_gama_tag(pb, track, mov->gamma);
        else
            av_log(mov->fc, AV_LOG_WARNING, "Not writing 'gama' atom. Format is not MOV.\n");
    }
    if (mov->flags & FF_MOV_FLAG_WRITE_COLR) {//cwm false
    ...
    if (track->enc->sample_aspect_ratio.den && track->enc->sample_aspect_ratio.num &&
        track->enc->sample_aspect_ratio.den != track->enc->sample_aspect_ratio.num) {//cwm false
...
static int mov_write_stsd_tag(AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    if (track->enc->codec_type == AVMEDIA_TYPE_VIDEO)
        mov_write_video_tag(pb, mov,  track);//cwm true
    else if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO)
...
static int mov_write_stts_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO && !track->audio_vbr) {//cwm false
        ...
        if (track->entry) {//cwm true track->entry=232
            stts_entries = av_malloc_array(track->entry, sizeof(*stts_entries)); /* worst case */
            if (!stts_entries)
                return AVERROR(ENOMEM);
        }
        for (i = 0; i < track->entry; i++) {
            int duration = get_cluster_duration(track, i);//cwm 计算下一个cluster的dts与前一个dts的差值
            if (i && duration == stts_entries[entries].duration) {//stts_entries负责保存duration，以及相同duration的个数保存到count
                stts_entries[entries].count++; /* compress */
            } else {//count代表后面的cluster的dts值与当前dts差值相同的个数
                entries++;
                stts_entries[entries].duration = duration;
                stts_entries[entries].count = 1;
            }
        }
        entries++; /* last one *///cwm entries <= track->entry
    }
    atom_size = 16 + (entries * 8);
    avio_wb32(pb, atom_size); /* size */
...
static int mov_write_stbl_tag(AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    mov_write_stsd_tag(pb, mov, track);//stsd->hev1->hvcC
    mov_write_stts_tag(pb, track);//cwm写stts, 计算相同的duration，且相同的duration使用同一个MOVStts，且MOVStts中的count记录相同的个数
    if ((track->enc->codec_type == AVMEDIA_TYPE_VIDEO ||
         track->enc->codec_tag == MKTAG('r','t','p',' ')) &&
        track->has_keyframes && track->has_keyframes < track->entry)
        mov_write_stss_tag(pb, track, MOV_SYNC_SAMPLE);//cwm MOV_SYNC_SAMPLE=1
    if (track->mode == MODE_MOV && track->flags & MOV_TRACK_STPS)
        mov_write_stss_tag(pb, track, MOV_PARTIAL_SYNC_SAMPLE);//cwm false
    if (track->enc->codec_type == AVMEDIA_TYPE_VIDEO &&
        track->flags & MOV_TRACK_CTTS && track->entry) {//cwm false

        if ((ret = mov_write_ctts_tag(pb, track)) < 0)
            return ret;
    }
    mov_write_stsc_tag(pb, track);//cwm 描述chunk的个数，以及每个chunk中包含多少个samples
    mov_write_stsz_tag(pb, track);//cwm 说明samples的个数，以及每个samples数据的长度
    mov_write_stco_tag(pb, track);//cwm
    return update_size(pb, pos);
}
...
static int mov_write_hdlr_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    int64_t pos = avio_tell(pb);//cwm 2393174
    ...
    if (track) {
        hdlr = (track->mode == MODE_MOV) ? "mhlr" : "\0\0\0\0";//cwm hdlr = "\0\0\0\0"
        if (track->enc->codec_type == AVMEDIA_TYPE_VIDEO) {//cwm true
            hdlr_type = "vide";
            descr     = "VideoHandler";
        } else if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO) {
            hdlr_type = "soun";
...
static int mov_write_minf_tag(AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    if (track->enc->codec_type == AVMEDIA_TYPE_VIDEO)
        mov_write_vmhd_tag(pb);//cwm true
    else if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO)
    ...
    mov_write_dinf_tag(pb);//cwm true
    if ((ret = mov_write_stbl_tag(pb, mov, track)) < 0)//cwm 
        return ret;
    return update_size(pb, pos);
}
...
static int mov_write_mdhd_tag(AVIOContext *pb, MOVMuxContext *mov,
                              MOVTrack *track)
{
    int version = track->track_duration < INT32_MAX ? 0 : 1;//cwm 0

    if (track->mode == MODE_ISM)
        version = 1;

    (version == 1) ? avio_wb32(pb, 44) : avio_wb32(pb, 32); /* size *///cwm 32
...
static int mov_write_mdia_tag(AVIOContext *pb, MOVMuxContext *mov,
                              MOVTrack *track)
{
    int64_t pos = avio_tell(pb);//2393134
    int ret;
                              ...
                              MOVTrack *track, AVStream *st)
{
    ...
    int version = duration < INT32_MAX ? 0 : 1;//version = 0，用来判断是32位还是64位
    ...
    if (track->flags & MOV_TRACK_ENABLED)//cwm true
    ...
    if (display_matrix) {
        ...
        write_matrix(pb,  1,  0,  0,  1, 0, 0);//cwm true
    }
    ...
    if (st && (track->enc->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (track->mode == MODE_MOV) {
            ...
            int64_t track_width_1616 = av_rescale(st->sample_aspect_ratio.num,
                                                  ...
                                                  st->sample_aspect_ratio.den);//cwm 0
            if (!track_width_1616 ||
...
{
    int64_t duration = av_rescale_rnd(track->track_duration, MOV_TIMESCALE,
                                      track->timescale, AV_ROUND_UP);//cwm 4660 = 0x1234
    int version = duration < INT32_MAX ? 0 : 1;//cwm 0
    int entry_size, entry_count, size;
    int64_t delay, start_ct = track->start_cts;//cwm 0
    int64_t start_dts = track->start_dts;//cwm 0

    if (track->entry) {//track->entry = 232,for tiandy.mp4
        if (start_dts != track->cluster[0].dts || start_ct != track->cluster[0].cts) {//cwm false
    ...
    delay = av_rescale_rnd(start_dts + start_ct, MOV_TIMESCALE,
                           track->timescale, AV_ROUND_DOWN);//cwm 0
    version |= delay < INT32_MAX ? 0 : 1;//cwm 0

    entry_size = (version == 1) ? 20 : 12;//cwm 12
    entry_count = 1 + (delay > 0);//cwm 1
    size = 24 + entry_count * entry_size;//cwm 36

    /* write the atom data */
    avio_wb32(pb, size);//cwm 36=0x24
                                    ...
                                    AVStream *st)
{
    ...
    if (mov->mode & MODE_MP4)
        mov_write_track_metadata(pb_buf, st, "name", "title");//cwm return 0

    if ((size = avio_close_dyn_buf(pb_buf, &buf)) > 0) {//cwm false
        avio_wb32(pb, size + 8);
        ffio_wfourcc(pb, "udta");
        avio_write(pb, buf, size);
    }
...
static int mov_write_trak_tag(AVIOContext *pb, MOVMuxContext *mov,
                              MOVTrack *track, AVStream *st)
{
    int64_t pos = avio_tell(pb);//pos = moov_pos + moov_head_size + mvhd_head_size = 2392882 + 8 + 108 = 2392998，pos得到trak_tag要写的位置
    int entry_backup = track->entry;// 232 for tiandy.h265
    int chunk_backup = track->chunkCount;// 3 for tiandy.h265
    int ret;
    ...
    if (track->start_dts != AV_NOPTS_VALUE) {//cwm true
        if (mov->use_editlist)//cwm true, 1
    ...
    if (track->tref_tag)//cwm false
        mov_write_tref_tag(pb, track);

    if ((ret = mov_write_mdia_tag(pb, mov, track)) < 0)//cwm 
        return ret;
    if (track->mode == MODE_PSP)//cwm false
        mov_write_uuid_tag_psp(pb, track); // PSP Movies require this uuid box
    if (track->tag == MKTAG('r','t','p',' '))//cwm false
    ...
    mov_write_track_udta_tag(pb, mov, st);//cwm  false
...
static int mov_write_mvhd_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].entry > 0 && mov->tracks[i].timescale) {
            int64_t max_track_len_temp = av_rescale_rnd(mov->tracks[i].track_duration,
                                                ...
                                                AV_ROUND_UP);//max_track_len_temp = 2392882
    ...
    version = max_track_len < UINT32_MAX ? 0 : 1;//cwm version = 0
    avio_wb32(pb, version == 1 ? 120 : 108); /* size *///108
    ...
    if (version == 1) {
        ...
        avio_wb32(pb, mov->time); /* creation time *///cwm true
        avio_wb32(pb, mov->time); /* modification time *///cwm true
    }
                              ...
                              AVFormatContext *s)
{
    ...
    mov_write_string_metadata(s, pb, "\251nam", "title"    , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251ART", "artist"   , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "aART", "album_artist", 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251wrt", "composer" , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251alb", "album"    , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251day", "date"     , 1);//cwm return 0
    if (!mov_write_string_metadata(s, pb, "\251too", "encoding_tool", 1)) {//cwm return 0
        if (!(s->flags & AVFMT_FLAG_BITEXACT))//cwm true
            mov_write_string_tag(pb, "\251too", LIBAVFORMAT_IDENT, 0, 1);
    }
    mov_write_string_metadata(s, pb, "\251cmt", "comment"  , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251gen", "genre"    , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251cpy", "copyright", 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251grp", "grouping" , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "\251lyr", "lyrics"   , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "desc",    "description",1);//cwm return 0
    mov_write_string_metadata(s, pb, "ldes",    "synopsis" , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "tvsh",    "show"     , 1);//cwm return 0
    mov_write_string_metadata(s, pb, "tven",    "episode_id",1);//cwm return 0
    mov_write_string_metadata(s, pb, "tvnn",    "network"  , 1);//cwm return 0
    mov_write_int8_metadata  (s, pb, "tves",    "episode_sort",4);//cwm return 0
    mov_write_int8_metadata  (s, pb, "tvsn",    "season_number",4);//cwm return 0
    mov_write_int8_metadata  (s, pb, "stik",    "media_type",1);//cwm return 0
    mov_write_int8_metadata  (s, pb, "hdvd",    "hd_video",  1);//cwm return 0
    mov_write_int8_metadata  (s, pb, "pgap",    "gapless_playback",1);//cwm return 0
    mov_write_int8_metadata  (s, pb, "cpil",    "compilation", 1);//cwm return 0
    mov_write_trkn_tag(pb, mov, s, 0); // track number //cwm return 0
    mov_write_trkn_tag(pb, mov, s, 1); // disc number //cwm return 0
    mov_write_tmpo_tag(pb, s);//cwm return 0
    return update_size(pb, pos);
}
                              ...
                              AVFormatContext *s)
{
    ...
    if (mov->mode & MODE_3GP) {
        ...
        /* iTunes meta data */
        mov_write_meta_tag(pb_buf, mov, s);//cwm true
    }

    if (s->nb_chapters && !(mov->flags & FF_MOV_FLAG_DISABLE_CHPL))//cwm false
...
//cwm 把每个sample划归到指定的chunk中去，每个chunk中包含samples的数据大小最大为1MB，若当前的samples加上之前的samples数据大小所得的值>=1MB,则以此samples开始重新建立新chunk
static void build_chunks(MOVTrack *trk)
{//在mov_write_moov_tag中被调用
    int i;
    ...
    if (trk->chunkCount)
        return;
    trk->chunkCount = 1;
    for (i = 1; i<trk->entry; i++){//trk->entry = 232
        if (chunk->pos + chunkSize == trk->cluster[i].pos &&
            chunkSize + trk->cluster[i].size < (1<<20)){//cwm chunk->pos + chunkSize == trk->cluster[i].pos判断每个cluster中的size、pos是否正确，且一个chunk包含的数据最大为1MB
            chunkSize             += trk->cluster[i].size;
            chunk->samples_in_chunk += trk->cluster[i].entries;//cwm samples_in_chunk代表chunk中的采样数量
        } else {
            trk->cluster[i].chunkNum = chunk->chunkNum+1;
            chunk=&trk->cluster[i];//如果cluster中的信息错误或一个chunk包含数据的大小已经超过了1MB,则重新开始一个新的chunk计算
            chunkSize = chunk->size;
            trk->chunkCount++;
        }
    }
}
//cwm 写"moov"标志、计算chunk数量，chunk的信息在chunk的第一个samples中体现,samples类型为MOVIentry
static int mov_write_moov_tag(AVIOContext *pb, MOVMuxContext *mov,
                              AVFormatContext *s)
{
    int i;
    int64_t pos = avio_tell(pb);//cwm pos为开始写moov box的位置
    avio_wb32(pb, 0); /* size placeholder*///cwm 写4个字节的0
    ffio_wfourcc(pb, "moov");//写4个字节的"moov"

    for (i = 0; i < mov->nb_streams; i++) {//mov->nb_streams = 0
        ...
        if (mov->tracks[i].entry)
            build_chunks(&mov->tracks[i]);//cwm 通过之前的samples，创建chunks，每个chunks包含的数据不超过1MB
    }

    if (mov->chapter_track)//cwm false
        for (i = 0; i < s->nb_streams; i++) {
            mov->tracks[i].tref_tag = MKTAG('c','h','a','p');
            mov->tracks[i].tref_id  = mov->tracks[mov->chapter_track].track_id;
        }
    for (i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].tag == MKTAG('r','t','p',' ')) {//cwm false
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].tag == MKTAG('t','m','c','d')) {//cwm false
    ...
    mov_write_mvhd_tag(pb, mov);//写mvhd box，大小为108字节
    if (mov->mode != MODE_MOV && !mov->iods_skip)//cwm false
        mov_write_iods_tag(pb, mov);
    for (i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].entry > 0 || mov->flags & FF_MOV_FLAG_FRAGMENT) {//mov->tracks[i].entry = 232
            int ret = mov_write_trak_tag(pb, mov, &(mov->tracks[i]), i < s->nb_streams ? s->streams[i] : NULL);
            if (ret < 0)
                return ret;
        }
    }
    if (mov->flags & FF_MOV_FLAG_FRAGMENT)//cwm false
        mov_write_mvex_tag(pb, mov); /* QuickTime requires trak to precede this */

    if (mov->mode == MODE_PSP)
        mov_write_uuidusmt_tag(pb, s);
    else
        mov_write_udta_tag(pb, mov, s);//cwm true
...
{
    avio_wb32(pb, 8);    // placeholder for extended size field (64 bit)// cwm 0x00000008
    ffio_wfourcc(pb, mov->mode == MODE_MOV ? "wide" : "free");//cwm "free"

    mov->mdat_pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size placeholder*/
    ffio_wfourcc(pb, "mdat");//cwm "mdat"
    return 0;
}
...
static int mov_write_ftyp_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    avio_wb32(pb, 0); /* size */ //先写了4字节的0
    ffio_wfourcc(pb, "ftyp");//cwm
...
static int mov_write_identification(AVIOContext *pb, AVFormatContext *s)
{
    ...
    mov_write_ftyp_tag(pb,s);//cwm ftyp
    if (mov->mode == MODE_PSP) {//cwm false for mp4
...
//cwm 解析h265数据，封装mp4格式
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{//pkt->data是nal数据源src，s是目的数据，nal单元的大小以及数据写到了s->pb->buf_ptr
    MOVMuxContext *mov = s->priv_data;
    AVIOContext *pb = s->pb;
    ...
    if (enc->codec_id == AV_CODEC_ID_AMR_NB) {//cwm false
    ...
    } else if (trk->sample_size)//cwm
        samples_in_chunk = size / trk->sample_size;
    else
        samples_in_chunk = 1;//cwm 

    /* copy extradata if it exists */
    if (trk->vos_len == 0 && enc->extradata_size > 0 &&
        !TAG_IS_AVCI(trk->tag) &&
        (enc->codec_id != AV_CODEC_ID_DNXHD)) {//cwm false
    ...
    if (enc->codec_id == AV_CODEC_ID_AAC && pkt->size > 2 &&
        (AV_RB16(pkt->data) & 0xfff0) == 0xfff0) {//cwm false
    ...
    if (enc->codec_id == AV_CODEC_ID_H264 && trk->vos_len > 0 && *(uint8_t *)trk->vos_data != 1 && !TAG_IS_AVCI(trk->tag)) {
        /* from x264 or from bytestream h264 */ //cwm false
        ...
        if (trk->hint_track >= 0 && trk->hint_track < mov->nb_streams) {
            ...
            size = ff_avc_parse_nal_units(pb, pkt->data, pkt->size);
               ...
               (AV_RB24(trk->vos_data) == 1 || AV_RB32(trk->vos_data) == 1)) {//cwm true
        /* extradata is Annex B, assume the bitstream is too and convert it */
        if (trk->hint_track >= 0 && trk->hint_track < mov->nb_streams) {
            ff_hevc_annexb2mp4_buf(pkt->data, &reformatted_data, &size, 0, NULL);
            avio_write(pb, reformatted_data, size);
        } else {
            size = ff_hevc_annexb2mp4(pb, pkt->data, pkt->size, 0, NULL);//cwm 封装h265数据为mp4格式pkt->size=45978 11380
        }
#if CONFIG_AC3_PARSER
    ...
    if ((enc->codec_id == AV_CODEC_ID_DNXHD ||
         enc->codec_id == AV_CODEC_ID_AC3) && !trk->vos_len) {//cwm false
    ...
    if (trk->entry >= trk->cluster_capacity) {//cwm true
    ...
    trk->cluster[trk->entry].pos              = avio_tell(pb) - size;//cwm
    trk->cluster[trk->entry].samples_in_chunk = samples_in_chunk;
    trk->cluster[trk->entry].chunkNum         = 0;
    trk->cluster[trk->entry].size             = size;
    trk->cluster[trk->entry].entries          = samples_in_chunk;
    trk->cluster[trk->entry].dts              = pkt->dts;
    if (!trk->entry && trk->start_dts != AV_NOPTS_VALUE) {//cwm false
        if (!trk->frag_discont) {
    ...
    if (trk->start_dts == AV_NOPTS_VALUE) {//cwm
    ...
    if (pkt->dts != pkt->pts)//cwm false
        trk->flags |= MOV_TRACK_CTTS;
    trk->cluster[trk->entry].cts   = pkt->pts - pkt->dts;
    trk->cluster[trk->entry].flags = 0;
    if (trk->start_cts == AV_NOPTS_VALUE)
        trk->start_cts = pkt->pts - pkt->dts;//cwm
    if (trk->end_pts == AV_NOPTS_VALUE)
        trk->end_pts = trk->cluster[trk->entry].dts +
                       trk->cluster[trk->entry].cts + pkt->duration;//cwm
    else
        trk->end_pts = FFMAX(trk->end_pts, trk->cluster[trk->entry].dts +
                                           trk->cluster[trk->entry].cts +
                                           pkt->duration);

    if (enc->codec_id == AV_CODEC_ID_VC1) {//cwm false
        mov_parse_vc1_frame(pkt, trk);
    } else if (pkt->flags & AV_PKT_FLAG_KEY) {//cwm
...
static int mov_write_single_packet(AVFormatContext *s, AVPacket *pkt)
{
        ...
        return ff_mov_write_packet(s, pkt);//cwm 下一级调用处
}
//cwm提取h265数据到mp4格式, 上一级mov_write_packet
static int mov_write_subtitle_end_packet(AVFormatContext *s,
    ...
    ret = mov_write_single_packet(s, &end);//cwm 下一级调用处, 解析h265到mp4格式
    av_packet_unref(&end);

    return ret;
}
//在libavformat/mux.c 的第660行处调用write_packet
static int mov_write_packet(AVFormatContext *s, AVPacket *pkt)//cwm 
{//与mov_write_header对应
    if (!pkt) {
        ...
        int i;
        MOVMuxContext *mov = s->priv_data;//cwm 
        ...
        for (i = 0; i < mov->nb_streams; i++) {
            ...
            if (trk->enc->codec_id == AV_CODEC_ID_MOV_TEXT &&
                ...
                ret = mov_write_subtitle_end_packet(s, i, trk->track_duration);//cwm 
                if (ret < 0) return ret;
                trk->last_sample_is_subtitle_end = 1;
            }
        }

        return mov_write_single_packet(s, pkt);//cwm
    }
}
    ...
    for (i = 0; i < AVMEDIA_TYPE_NB; i++) {
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            if (!enabled[i] && first[i] >= 0)
                mov->tracks[first[i]].flags |= MOV_TRACK_ENABLED;//cwm =4
            break;
        }
    }
}
...
//与mov_write_packet对应
static int mov_write_header(AVFormatContext *s)//cwm 调用出在 avformat_write_header
{
    AVIOContext *pb = s->pb;
    MOVMuxContext *mov = s->priv_data;//访问方法p *((MOVMuxContext *)output_files[0]->ctx->priv_data)
    ...
    if (!(mov->flags & FF_MOV_FLAG_DELAY_MOOV)) {
        if ((ret = mov_write_identification(pb, s)) < 0)//cwm write vbox tag ftyp ...
            return ret;
    }
    ...
    if (mov->flags & FF_MOV_FLAG_RTP_HINT) {//cwm false
    ...
    if (mov->mode == MODE_MOV) {//cwm false
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (track->language < 0)
            track->language = 0;
        track->mode = mov->mode;
        track->tag  = mov_find_codec_tag(s, track);//cwm tag = "hevc" = tag
        if (!track->tag) {
         ...
         * this is updated. */
        track->hint_track = -1;
        track->start_dts  = AV_NOPTS_VALUE;//cwm
        track->start_cts  = AV_NOPTS_VALUE;
        track->end_pts    = AV_NOPTS_VALUE;
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {//cwm false -> true
            ...
            if (mov->video_track_timescale) {
                ...
                track->timescale = st->time_base.den;//cwm
            ...
            if (track->mode == MODE_MOV && track->timescale > 100000)//cwm true
                av_log(s, AV_LOG_WARNING,
        ...
        if (!track->height)
            track->height = st->codec->height;//cwm
        ...
        if (st->codec->extradata_size) {
            if (st->codec->codec_id == AV_CODEC_ID_DVD_SUBTITLE)
                ...
                track->vos_len  = st->codec->extradata_size;//cwm 82
    ...
    if (mov->flags & FF_MOV_FLAG_RTP_HINT) {//false cwm
    ...
    avio_flush(pb);//cwm write buffer content to file

    if (mov->flags & FF_MOV_FLAG_ISML)//false cwm
        mov_write_isml_manifest(pb, mov);

    if (mov->flags & FF_MOV_FLAG_EMPTY_MOOV &&
        !(mov->flags & FF_MOV_FLAG_DELAY_MOOV)) {//fase cwm
...
//在mux.c的1010行被调用，用来计算tiandy.mp4文件的各个box,且把各个box按一定规则写到文件合适的位置，box的数据是连着的
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    if (!(mov->flags & FF_MOV_FLAG_FRAGMENT)) {
        moov_pos = avio_tell(pb);//moov_pos是mp4中moov box所在文件的位置，2392882
		//mov->mdat_size = moov_pos + 44
        /* Write size of mdat tag */
        if (mov->mdat_size + 8 <= UINT32_MAX) {
            avio_seek(pb, mov->mdat_pos, SEEK_SET);//cwm mov->mdat_pos = 36
            avio_wb32(pb, mov->mdat_size + 8);//2392838，cwm mov->mdat_size表示h265的数据封装为mp4数据部分的大小(不包含box等头信息)
        } else {
		...
		//把tiandy.mp4的文件写指针定位到moov_pos=2392882，即数据box开始的位置
        if (mov->flags & FF_MOV_FLAG_FASTSTART) {
        ...
        } else if (mov->reserved_moov_size > 0) {//cwm mov->reserved_moov_size = 0
            int64_t size;
            if ((res = mov_write_moov_tag(pb, mov, s)) < 0)
                goto error;
            ...
            if ((res = mov_write_moov_tag(pb, mov, s)) < 0)//cwm true
                goto error;
        }
        res = 0;
    } else {
...
AVOutputFormat ff_mp4_muxer = {
    ...
    .video_codec       = CONFIG_LIBX264_ENCODER ?
                         AV_CODEC_ID_H264 : AV_CODEC_ID_MPEG4,
    .write_header      = mov_write_header,//cwm 调用出在 avformat_write_header
    .write_packet      = mov_write_packet,
    .write_trailer     = mov_write_trailer,
    .flags             = AVFMT_GLOBALHEADER | AVFMT_ALLOW_FLUSH | AVFMT_TS_NEGATIVE,
