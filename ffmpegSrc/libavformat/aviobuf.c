...
static void writeout(AVIOContext *s, const uint8_t *data, int len)
{
    if (s->write_packet && !s->error) {
        int ret = s->write_packet(s->opaque, (uint8_t *)data, len);//cwm  s->write_packet = ffurl_write
        if (ret < 0) {
            s->error = ret;
        }
    }
...
int64_t avio_size(AVIOContext *s)
{
    ...
    if (size < 0) {
        if ((size = s->seek(s->opaque, -1, SEEK_END)) < 0)//cwm ffurl_seek
            return size;
        size++;
...
static void fill_buffer(AVIOContext *s)
{
    int max_buffer_size = s->max_packet_size ?
                          s->max_packet_size : IO_BUFFER_SIZE;//cwm 32768
    uint8_t *dst        = s->buf_end - s->buffer + max_buffer_size < s->buffer_size ?
                          s->buf_end : s->buffer;
    int len             = s->buffer_size - (dst - s->buffer);//cwm 计算读取文件的字节数,32768
    ...
    if (s->read_packet)
        len = s->read_packet(s->opaque, dst, len);//cwm ffurl_read
    else
        len = 0;
    if (len <= 0) {
...
//cwm 读取文件，在libavformat/format.c:287被调用
int avio_read(AVIOContext *s, unsigned char *buf, int size)
{
    ...
    while (size > 0) {
        ...
        if (len > size)
            len = size;
        if (len == 0 || s->write_flag) {//cwm write
            if((s->direct || size > s->buffer_size) && !s->update_checksum){
                ...
                fill_buffer(s);//cwm 下一级调用ffurl_read
                len = s->buf_end - s->buf_ptr;
                if (len == 0)
                    break;
            }
        } else {
            memcpy(buf, s->buf_ptr, len);// cwm read
            buf += len;
            s->buf_ptr += len;
            size -= len;
        }
...
int ffio_read_partial(AVIOContext *s, unsigned char *buf, int size)
{
    ...
    if (s->read_packet && s->write_flag) {
        len = s->read_packet(s->opaque, buf, size);//cwm ffurl_read
        if (len > 0)
            s->pos += len;
        return len;
    }
