                                                              ...
                                                              int size))
{
    ...
    while (len < size_min) {
        if (ff_check_interrupt(&h->interrupt_callback))
            return AVERROR_EXIT;
        ret = transfer_func(h, buf + len, size - len);//cwm file_read
...
//cwm 读输入文件libavformat/aviobuf.c:480处被调用
int ffurl_read(URLContext *h, unsigned char *buf, int size)
{
    if (!(h->flags & AVIO_FLAG_READ))
        return AVERROR(EIO);
    return retry_transfer_wrapper(h, buf, size, 1, h->prot->url_read);//cwm h->prot->url_read = file_read
}
...
//cwm 
int ffurl_write(URLContext *h, const unsigned char *buf, int size)//cwm buf源数据,size写大小
{
    ...
    return retry_transfer_wrapper(h, (unsigned char *)buf, size, size,
                                  ...
                                  h->prot->url_write);//cwm h->prot->url_write = file_write
}
//cwm 在ffurl_connect、avio_size被调用过
int64_t ffurl_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    ret = h->prot->url_seek(h, pos, whence & ~AVSEEK_FORCE);//cwm h->prot->url_seek = file_seek
    return ret;
}
