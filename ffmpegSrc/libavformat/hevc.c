...
static int hvcc_write(AVIOContext *pb, HEVCDecoderConfigurationRecord *hvcc)
{
    ...
    av_log(NULL, AV_LOG_TRACE,  "numOfArrays:                         %"PRIu8"\n",
            hvcc->numOfArrays);
    for (i = 0; i < hvcc->numOfArrays; i++) {//hvcc->numOfArrays = 3
        ...
        for (j = 0; j < hvcc->array[i].numNalus; j++)//cwm hvcc->array[i].numNalus = 1
...
//cwm ·â×°hevcÊý¾ÝÎªmp4¸ñÊ½Ê±£¬½âÎöµÄº¯Êý
                       ...
                       int size, int filter_ps, int *ps_count)
{
    ...
    if (!filter_ps) {
        ret = ff_avc_parse_nal_units(pb, buf_in, size);//cwm
        goto end;
    }
                           ...
                           int *size, int filter_ps, int *ps_count)
{
    ...
    ret   = ff_hevc_annexb2mp4(pb, buf_in, *size, filter_ps, ps_count);//cwm
