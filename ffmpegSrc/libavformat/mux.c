...
int avformat_write_header(AVFormatContext *s, AVDictionary **options)//cwm
{
    int ret = 0;
    ...
    if (s->oformat->write_header) {
        ret = s->oformat->write_header(s);//cwm path:libavformat/movenc.c mov_write_header
...
//        pkt->pts= st->cur_dts;
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        break;
    case AVMEDIA_TYPE_VIDEO:
        frac_add(st->priv_pts, (int64_t)st->time_base.den * st->codec->time_base.num);//cwm true
        break;
    }
    return 0;
}
 ...
 */
static int write_packet(AVFormatContext *s, AVPacket *pkt)//967处调用
{
    int ret, did_split;
    ...
    if ((pkt->flags & AV_PKT_FLAG_UNCODED_FRAME)) {
        ...
        ret = s->oformat->write_packet(s, pkt);//cwm mov_write_packet 
    }
                                 ...
                                 AVPacket *pkt, int flush)
{
    ...
    if (stream_count && flush) {
        AVStream *st;
        pktl = s->internal->packet_buffer;//cwm
        *out = pktl->pkt;//cwm out->size为从输入文件中读取数据的字节数
...
{
    if (s->oformat->interleave_packet) {//cwm false
        int ret = s->oformat->interleave_packet(s, out, in, flush);
        if (in)
            av_packet_unref(in);
        return ret;
    } else
        return ff_interleave_packet_per_dts(s, out, in, flush);//cwm true
}
//cwm 在ffmpeg.c write_frame 774行被调用
int av_interleaved_write_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt) {
        ...
        if ((ret = compute_pkt_fields2(s, st, pkt)) < 0 && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))//cwm true
            goto fail;
    ...
    for (;; ) {
        AVPacket opkt;
        int ret = interleave_packet(s, &opkt, pkt, flush);//h265 nal数据保存在了opkt.stream_index
        if (pkt) {
        ...
        ret = write_packet(s, &opkt);//cwm 写h265包数据,在本文件的592行
        if (ret >= 0)
...
fail:
    if (s->oformat->write_trailer)
        if (ret >= 0) {//cwm h265的数据写完后，写描述265数据的各个box
        ret = s->oformat->write_trailer(s);//cwm s->oformat->write_trailer =  mov_write_trailer
        } else {
