...
static int64_t wrap_timestamp(AVStream *st, int64_t timestamp)
{
    if (st->pts_wrap_behavior != AV_PTS_WRAP_IGNORE &&
        st->pts_wrap_reference != AV_NOPTS_VALUE && timestamp != AV_NOPTS_VALUE) {//cwm false
                      ...
                      AVDictionary **options)
{
    ...
    if ((s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        ...
        return score;//cwm false

    if ((ret = avio_open2(&s->pb, filename, AVIO_FLAG_READ | s->avio_flags,
                          &s->interrupt_callback, options)) < 0)
        return ret;//cwm false
    if (s->iformat)
        return 0;//cwm false
    return av_probe_input_buffer2(s->pb, &s->iformat, filename,
                                 s, 0, s->format_probesize);
}
//cwm ref=1是向packet_buffer链表放pkt节点，ref=0是从packet_buffer链表取节点到pkt
                        ...
                        AVInputFormat *fmt, AVDictionary **options)
{
    ...
    if ((ret = init_input(s, filename, &tmp)) < 0)//探测输入文件的格式
        goto fail;
    s->probe_score = ret;
    ...
    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header)
        if ((ret = s->iformat->read_header(s)) < 0)//cwm ff_raw_video_read_header
            goto fail;
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        pkt->data = NULL;
        pkt->size = 0;
        av_init_packet(pkt);//AVPacket 成员变量的初始化
        ret = s->iformat->read_packet(s, pkt);//cwm s->iformat->read_packet = ff_raw_read_partial_packet
        if (ret < 0) {
        ...
        if ((s->flags & AVFMT_FLAG_DISCARD_CORRUPT) &&
            (pkt->flags & AV_PKT_FLAG_CORRUPT)) {//cwm false
            av_log(s, AV_LOG_WARNING,
        ...
        if (pkt->stream_index >= (unsigned)s->nb_streams) {//cwm false
        ...
        if (update_wrap_reference(s, st, pkt->stream_index, pkt) && st->pts_wrap_behavior == AV_PTS_WRAP_SUB_OFFSET) {//cwm false
        ...
        err = add_to_pktbuf(&s->internal->raw_packet_buffer, pkt,
                            &s->internal->raw_packet_buffer_end, 0);//cwm 把pkt放到struct AVPacketList *raw_packet_buffer链表中
        if (err)
            return err;
        s->internal->raw_packet_buffer_remaining_size -= pkt->size;
...
static int parse_packet(AVFormatContext *s, AVPacket *pkt, int stream_index)
{
    ...
    while (size > 0 || (pkt == &flush_pkt && got_output)) {
        ...
        ret = add_to_pktbuf(&s->internal->parse_queue, &out_pkt,
                            &s->internal->parse_queue_end, 1);//向s->internal->parse_queue链表放入out_pkt节点
...
//cwm parse_packet
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)//cwm
{
    ...
    while (!got_packet && !s->internal->parse_queue) {
        ...
        /* read next packet */
        ret = ff_read_packet(s, &cur_pkt);//cwm 读取数据
        if (ret < 0) {
        ...
        if (cur_pkt.pts != AV_NOPTS_VALUE &&
            ...
            cur_pkt.pts < cur_pkt.dts) {//cwm false
            av_log(s, AV_LOG_WARNING,
        ...
        if (s->debug & FF_FDEBUG_TS)//cwm false
            av_log(s, AV_LOG_DEBUG,
        ...
        if (st->need_parsing && !st->parser && !(s->flags & AVFMT_FLAG_NOPARSE)) {//cwm false
        ...
        if (!st->need_parsing || !st->parser) {//cwm false
            ...
            *pkt = cur_pkt;
            ...
            got_packet = 1;
        } else if (st->discard < AVDISCARD_ALL) {//链表队列AVPacketList parse_queue
            if ((ret = parse_packet(s, &cur_pkt, cur_pkt.stream_index)) < 0)//cwm 解析数据且把数据放入 链表队列AVPacketList，s->internal->parse_queue
                return ret;
        } else {
            /* free packet */
    ...
    if (!got_packet && s->internal->parse_queue)//cwm 从AVPacketList parse_queue链表中取一个数据包，且把pkt指向这个数据包
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)//cwm
{
...
static void estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    if (ic->iformat->flags & AVFMT_NOFILE) {
        ...
        file_size = avio_size(ic->pb);//得到输入文件tiandy.h265的大小，字节为单位
...
/* returns 1 or 0 if or if not decoded data was returned, or a negative error *///cwm
static int try_decode_frame(AVFormatContext *s, AVStream *st, AVPacket *avpkt,
                            AVDictionary **options)
{//cwm libavformat/utils.c 3389行
    const AVCodec *codec;
...
//cwm 在ffmpeg_opt.c/open_input_file被调用, 解析nal数据函数read_frame_internal
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)//cwm
{
    int i, count, ret = 0, j;
    int64_t read_size;
    AVStream *st;
...
/*            if (!st->time_base.num)
    ...
    for (;;) {
         ...
         * (AVFMTCTX_NOHEADER). */
        ret = read_frame_internal(ic, &pkt1);//cwm true
        if (ret == AVERROR(EAGAIN))
            continue;
        ...
        if (!(ic->flags & AVFMT_FLAG_NOBUFFER)) {
            ret = add_to_pktbuf(&ic->internal->packet_buffer, pkt,
                                &ic->internal->packet_buffer_end, 0);//cwm
            if (ret < 0)
        ...
        try_decode_frame(ic, st, pkt,
                         (options && i < orig_nb_streams) ? &options[i] : NULL);//cwm 开始解析tiandy.h265文件
