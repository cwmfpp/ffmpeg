...
//cwm 写h265数据到mp4文件，使用pkt的形式
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    AVBitStreamFilterContext *bsfc = ost->bitstream_filters;
    AVCodecContext          *avctx = ost->encoding_needed ? ost->enc_ctx : ost->st->codec;//ost->encoding_needed = 0
    int ret;

    if (!ost->st->codec->extradata_size && ost->enc_ctx->extradata_size) {//cwm false
    ...
    if ((avctx->codec_type == AVMEDIA_TYPE_VIDEO && video_sync_method == VSYNC_DROP) ||
        (avctx->codec_type == AVMEDIA_TYPE_AUDIO && audio_sync_method < 0))//cwm false
    ...
    if (!(avctx->codec_type == AVMEDIA_TYPE_VIDEO && avctx->codec)) {
        if (ost->frame_number >= ost->max_frames) {//cwm false
            av_packet_unref(pkt);
            return;
        }
        ost->frame_number++;//cwm true
    }
    if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        int i;
        uint8_t *sd = av_packet_get_side_data(pkt, AV_PKT_DATA_QUALITY_STATS,
                                              NULL);
        ost->quality = sd ? AV_RL32(sd) : -1;// -1
        ost->pict_type = sd ? sd[4] : AV_PICTURE_TYPE_NONE;//sd = 0

        for (i = 0; i<FF_ARRAY_ELEMS(ost->error); i++) {
            if (sd && i < sd[5])
                ost->error[i] = AV_RL64(sd + 8 + 8*i);
            else
                ost->error[i] = -1;//cwm true
        }
    }
    ...
    ret = av_interleaved_write_frame(s, pkt);//cwm 写h265数据到mp4文件
    if (ret < 0) {
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    total_size = avio_size(oc->pb);//cwm 下一级调用ffurl_seek
...
//cwm 拷贝h265流数据内容到mp4文件，不是拷贝的h265的参数集
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    if (ost->enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO)//cwm
        ost->sync_opts++;
    ...
    if (pkt->dts == AV_NOPTS_VALUE)
        opkt.dts = av_rescale_q(ist->dts, AV_TIME_BASE_Q, ost->st->time_base);//cwm
    else
    ...
    if (ost->st->codec->codec_type == AVMEDIA_TYPE_VIDEO &&
        ...
        (of->ctx->oformat->flags & AVFMT_RAWPICTURE)) {//cwm false
    ...
    write_frame(of->ctx, &opkt, ost);//cwm 写h265的帧数据到mp4文件
}
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    ret = avcodec_decode_video2(ist->dec_ctx,
                                decoded_frame, got_output, pkt);//cwm
...
{//cwm处理h265的文件数据到mp4文件
    ...
    for (i = 0; pkt && i < nb_output_streams; i++) {
        ...
        do_streamcopy(ist, ost, pkt);//cwm 拷贝h265数据到mp4文件，不是参数集vps sps pps sei
    }
...
//*((MOVMuxContext *)output_files[0]->ctx->priv_data)的初始化是在mov_write_header
static int transcode_init(void)//cwm
{
    int ret = 0, i, j, k;
    AVFormatContext *oc;
    OutputStream *ost;
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        ist = get_input_stream(ost);//cwm

        if (ost->attachment_filename)
            continue;

        enc_ctx = ost->stream_copy ? ost->st->codec : ost->enc_ctx;//cwm

        if (ist) {
            dec_ctx = ist->dec_ctx;//cwm
        ...
        if (ost->stream_copy) {//cwm ost->stream_copy代表的是-vcodec copy命令,生成编码所需的部分参数
            AVRational sar;
            uint64_t extra_size;
            ...
            enc_ctx->codec_id   = dec_ctx->codec_id;//cwm AV_CODEC_ID_HEVC
            ...
            if (dec_ctx->extradata_size) {
                ...
                memcpy(enc_ctx->extradata, dec_ctx->extradata, dec_ctx->extradata_size);//cwm 拷贝文件tiandy.h265数据
            }
            enc_ctx->extradata_size= dec_ctx->extradata_size;
            enc_ctx->bits_per_coded_sample  = dec_ctx->bits_per_coded_sample;

            enc_ctx->time_base = ist->st->time_base;//cwm 配置好需要编码文件的信息，图像、码率、数据信息等
            /*
             * Avi is a special case here because it supports variable fps but
             * having the fps and timebase differe significantly adds quite some
             * overhead
             */        
            if(!strcmp(oc->oformat->name, "avi")) {//cwm oc->oformat->name == "mp4" //cwm false
            ...
            } else if(!(oc->oformat->flags & AVFMT_VARIABLE_FPS)//cwm false
                      && strcmp(oc->oformat->name, "mov") && strcmp(oc->oformat->name, "mp4") && strcmp(oc->oformat->name, "3gp")
                      && strcmp(oc->oformat->name, "3g2") && strcmp(oc->oformat->name, "psp") && strcmp(oc->oformat->name, "ipod")
                      && strcmp(oc->oformat->name, "f4v")
            ) {
                if(   copy_tb<0 && dec_ctx->time_base.den//cwm false
            ...
            if (   enc_ctx->codec_tag == AV_RL32("tmcd")//cwm false
            ...
            if (!ost->frame_rate.num)//cwm
                ost->frame_rate = ist->framerate;
            if(ost->frame_rate.num)//cwm false
            ...
            ost->parser = av_parser_init(enc_ctx->codec_id);//cwm
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
    ...
    /* open each encoder */
    for (i = 0; i < nb_output_streams; i++) {//cwm true
    ...
    for (i = 0; i < nb_input_files; i++) {//cwm
        InputFile *ifile = input_files[i];
    ...
    /* open files and write file headers *///cwm avformat_write_header -> mov_write_header -> mov_write_identification -> mov_write_ftyp_tag
    for (i = 0; i < nb_output_files; i++) {//cwm
        oc = output_files[i]->ctx;
        oc->interrupt_callback = int_cb;//初始化了OutputFile下的AVFormatContext成员变量
        if ((ret = avformat_write_header(oc, &output_files[i]->opts)) < 0) {//cwm 写输出文件头信息tiandy.mp4
...
//         assert_avoptions(output_files[i]->opts);
 ...
 dump_format:
    /* dump the file output parameters - cannot be done before in case
       of stream copy */
    for (i = 0; i < nb_output_files; i++) {
        av_dump_format(output_files[i]->ctx, i, output_files[i]->ctx->filename, 1);//cwm 打印输出文件的信息
    }
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy)
            av_log(NULL, AV_LOG_INFO, " (copy)");//cwm
        else {
...
#if HAVE_PTHREADS
static void *input_thread(void *arg)//cwm
{
    InputFile *f = arg;
...
static int process_input(int file_index)
{
    ...
    process_input_packet(ist, &pkt, 0);//cwm 处理h265数据，且解析成mp4格式的数据
...
//transcode_step -> process_input -> process_input_packet -> do_streamcopy -> write_frame -> av_interleaved_write_frame -> mux.c/write_packet -> mov_write_packet -> mov_write_subtitle_end_packet -> mov_write_single_packet -> ff_mov_write_packet -> ff_hevc_annexb2mp4 -> 
    ret = process_input(ist->file_index);//开始处理h265的数据，且解析为mp4所要求的格式
    if (ret == AVERROR(EAGAIN)) {
        if (input_files[ist->file_index]->eagain)
            ost->unavailable = 1;
        return 0;
 ...
 */
static int transcode(void)//cwm h265数据解析与拷贝
{
    int ret, i;
    AVFormatContext *os;
    OutputStream *ost;
    ...
    ret = transcode_init();//初始化了OutputFile下了类型为AVFormatContext的成员变量ctx
    if (ret < 0)
        goto fail;
...
#if HAVE_PTHREADS
    if ((ret = init_input_threads()) < 0)//cwm create threads
        goto fail;
#endif

    while (!received_sigterm) {//cwm
        ...
        ret = transcode_step();//cwm 打包h265数据为mp4
        if (ret < 0) {
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {//cwm 写描述mp4文件的box信息
...
//transcode -> transcode_step -> process_input -> process_input_packet -> do_streamcopy -> write_frame -> av_interleaved_write_frame -> mux.c/write_packet -> mov_write_packet -> mov_write_subtitle_end_packet -> mov_write_single_packet -> ff_mov_write_packet -> ff_hevc_annexb2mp4 -> 
int main(int argc, char **argv)
{
    ...
    ret = ffmpeg_parse_options(argc, argv);//cwm 解析输入命令，输入文件编解码格式，同时生成输出文件，ffmpeg -i tiandy.h265 -vcodec tiandy.mp4
    if (ret < 0)
        exit_program(1);

    if (nb_output_files <= 0 && nb_input_files == 0) {//cwm nb_input_files 输出文件的个数
        show_usage();
...
//     }
    ...
    if (transcode() < 0)//cwm 开始解析数据，且写数据、描述数据的box信息到文件tiandy.mp4
        exit_program(1);
    ti = getutime() - ti;
    if (do_benchmark) {
