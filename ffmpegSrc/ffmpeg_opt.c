...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (o->nb_audio_sample_rate) {//cwm false
        av_dict_set_int(&o->g->format_opts, "sample_rate", o->audio_sample_rate[o->nb_audio_sample_rate - 1].u.i, 0);
    }
    if (o->nb_audio_channels) {//cwm false
    ...
    if (o->nb_frame_sizes) {//cwm false
        av_dict_set(&o->g->format_opts, "video_size", o->frame_sizes[o->nb_frame_sizes - 1].u.str, 0);
    }
    if (o->nb_frame_pix_fmts)//cwm false
	...
	//cwm ic->video_codec_id = AV_CODEC_ID_NONE
    ...
    if (!av_dict_get(o->g->format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE)) {
        av_dict_set(&o->g->format_opts, "scan_all_pmts", "1", AV_DICT_DONT_OVERWRITE);//cwm
        scan_all_pmts_set = 1;
    }
    /* open the input file with generic avformat function */
    err = avformat_open_input(&ic, filename, file_iformat, &o->g->format_opts);//cwm 探测文件的格式
    if (err < 0) {
    ...
    ret = avformat_find_stream_info(ic, opts);//cwm 开始解析h265文件,图片的大小格式, 解析vps sps pps sei等nal
    if (ret < 0) {
    ...
    av_dump_format(ic, nb_input_files, filename, 0);//cwm 打印输入文件的信息
...
static void choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    } else if (!strcmp(codec_name, "copy"))//cwm
        ost->stream_copy = 1;
    else {
...
{//cwm
    ...
    choose_encoder(o, oc, ost);//cwm 解析输出数据的格式 -vcodec copy
...
{//cwm
    ...
    ost = new_output_stream(o, oc, AVMEDIA_TYPE_VIDEO, source_index);//cwm 解析输出文件的格式
    st  = ost->st;
...
{//cwm
    ...
    if (!strcmp(file_oformat->name, "ffm") && !override_ffserver &&
        ...
        if (!o->subtitle_disable && (avcodec_find_encoder(oc->oformat->subtitle_codec) || subtitle_codec_name)) {//cwm false
            for (i = 0; i < nb_input_streams; i++)
...
loop_end:
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        ...
        /* open the file *///cwm create out file , tiandy.mp4，且使文件的写指针为0
...
{//cwm
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        ret = open_file(&o, g->arg);//g->arg = "tiandy.mp4"
        uninit_options(&o);//cwm
        if (ret < 0) {
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    /* open input files */
    ret = open_files(&octx.groups[GROUP_INFILE], "input", open_input_file);//cwm open_input_file解析出输入文件的编码格式
    if (ret < 0) {
    ...
    /* open output files */
    ret = open_files(&octx.groups[GROUP_OUTFILE], "output", open_output_file);//cwm create output files
    if (ret < 0) {
