...
{
    return codec && codec->decode;//cwm codec->decode = hevc_decode_frame
}
...
//cwm 被libavformat/utils.c 下的try_decode_frame函数调用
                                              ...
                                              const AVPacket *avpkt)
{
    ...
    *got_picture_ptr = 0;
    ...
    if ((avctx->codec->capabilities & AV_CODEC_CAP_DELAY) || avpkt->size ||
        ...
        int did_split = av_packet_split_side_data(&tmp);//cwm
        ...
        if (HAVE_THREADS && avctx->active_thread_type & FF_THREAD_FRAME)
            ret = ff_thread_decode_frame(avctx, picture, got_picture_ptr,
                                         &tmp);
        else {//解析h265数据，提取nal单元,提取到的数据保存到(HEVCContext *) (avctx->priv_data)->pkt->nals
            ret = avctx->codec->decode(avctx, picture, got_picture_ptr,
                                       &tmp);//cwm avctx->codec->decode = hevc_decode_frame
