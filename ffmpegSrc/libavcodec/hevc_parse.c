 ...
 */
static int hls_nal_unit(HEVCNAL *nal, AVCodecContext *avctx)//cwm
{
    GetBitContext *gb = &nal->gb;
    int nuh_layer_id;

    if (get_bits1(gb) != 0)//改变gb->index的值加1
                         ...
                         AVCodecContext *avctx, int is_nalff, int nal_length_size)
{
    ...
    while (length >= 4) {
        ...
        if (pkt->nals_allocated < pkt->nb_nals + 1) {
            int new_size = pkt->nals_allocated + 1;//代表新分配的nal buf的个数，增加一个新的nal buf单元
        ...
        consumed = ff_hevc_extract_rbsp(s, buf, extract_length, nal);//返回值consumed代表一个nal单元数据的	有效长度(包含包头，不包含nal开始标志0x000001)
        if (consumed < 0)
            return consumed;
