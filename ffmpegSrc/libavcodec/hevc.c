...
static int decode_nal_units(HEVCContext *s, const uint8_t *buf, int length)
{
    ...
    ret = ff_hevc_split_packet(s, &s->pkt, buf, length, s->avctx, s->is_nalff,
                               s->nal_length_size);//把buf中包含的所有开始标志的nal单元提取到s->pk
    if (ret < 0) {
        av_log(s->avctx, AV_LOG_ERROR,
...
//cwm 从avpkt->data的数据中提取出包含的所有nal单元，解析出nal的数据保存到s->pkt->nals中,个数为s->pkt->nals_allocated
static int hevc_decode_frame(AVCodecContext *avctx, void *data, int *got_output,
                             AVPacket *avpkt)
{
    int ret;
    HEVCContext *s = avctx->priv_data;//avctx->priv_data代表的是HEVCContext
    ...
    s->ref = NULL;
    ret    = decode_nal_units(s, avpkt->data, avpkt->size);//解析avpkt->data的数据提取出nal 数据且拷贝到了s->pkt->nals->rbsp_buffer中
    if (ret < 0)
        return ret;
    ...
    if (s->output_frame->buf[0]) {
        av_frame_move_ref(data, s->output_frame);//初始化AVFrame，s->output_frame
        *got_output = 1;
    }
...
static av_cold int hevc_decode_init(AVCodecContext *avctx)//cwm
{
    HEVCContext *s = avctx->priv_data;
    int ret;
    ...
    if (avctx->extradata_size > 0 && avctx->extradata) {//cwm false
    ...
    if((avctx->active_thread_type & FF_THREAD_FRAME) && avctx->thread_count > 1)
            ...
            s->threads_type = FF_THREAD_SLICE;//cwm
...
AVCodec ff_hevc_decoder = {
    ...
    .decode                = hevc_decode_frame,//在libavcode/utils.c下的avcodec_decode_video2函数内被调用
