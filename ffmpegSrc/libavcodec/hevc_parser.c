...
{//cwm
    ...
    for (;;) {
        ...
        h->nal_unit_type = (*buf >> 1) & 0x3f;//cwm nal type
        ...
        case NAL_VPS:
            ...
            break;
        case NAL_SPS:
            ff_hevc_decode_nal_sps(gb, avctx, ps, 1);//ff_hevc_decode_nal_sps
            break;
        case NAL_PPS:
        ...
        case NAL_CRA_NUT:
            ...
            sh->first_slice_in_pic_flag = get_bits1(gb);//得到slice的flag，取前4字节，左移1位
                      ...
                      const uint8_t *buf, int buf_size)
{
    ...
    if (avctx->extradata && !ctx->parsed_extradata) {//cwm false -> true
        parse_nal_units(s, avctx->extradata, avctx->extradata_size, avctx);
        ctx->parsed_extradata = 1;
    }

    if (s->flags & PARSER_FLAG_COMPLETE_FRAMES) {//cwm false
        next = buf_size;
    } else {//cwm
...
AVCodecParser ff_hevc_parser = {
    ...
    .parser_parse   = hevc_parse,//cwm 在av_parser_parse2被调用
